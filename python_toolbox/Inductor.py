...
class Inductor:
    ...
    def __init__(self, mymaterial):
        ...
        # rects_planar[:,:,0] = [[rect1_x_start, rect1_x_end], [rect1_y_start, rect1_y_end]]
        # rects_planar[:,:,1] = [[rect2_x_start, rect2_x_end], [rect2_y_start, rect2_y_end]]
    ...
    def calcCenters(self):
        ...
        if len(self.rects_planar) > 0:
            self.centers_planar = np.zeros((2, self.rects_planar.shape[2]))
            for i in range(self.rects_planar.shape[2]):
                self.centers_planar[:, i] = [np.mean(self.rects_planar[0, :, i]), np.mean(self.rects_planar[1, :, i])]
        # Axi
        if len(self.rects_axi) > 0:
            self.centers_axi = np.zeros((2, self.rects_axi.shape[2]))
            for i in range(self.rects_axi.shape[2]):
                self.centers_axi[:, i] = [np.mean(self.rects_axi[0, :, i]), np.mean(self.rects_axi[1, :, i])]
